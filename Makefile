...
.PHONY: help
help:
	@echo "$(BLUE)🔧 Ada Async_Result Library - Build System$(NC)"
...
metrics:
	...
	@if command -v $(GNATMETRIC) > /dev/null 2>&1; then \
		$(GNATMETRIC) -P async_result.gpr --output-dir=metrics/; \
		...
		echo "  Public functions: $$(grep -c 'function.*return' src/async_result.ads)"; \
		echo "  Public procedures: $$(grep -c 'procedure' src/async_result.ads)"; \
	fi
...
complexity:
	...
	@if command -v $(GNATCHECK) > /dev/null 2>&1; then \
		$(GNATCHECK) -P async_result.gpr -rules -from=.gnatcheck; \
		echo "$(GREEN)✅ Complexity analysis completed$(NC)"; \
	else \
		echo "$(YELLOW)⚠️  GNATcheck not available. Using basic analysis.$(NC)"; \
		echo "$(YELLOW)📊 Function count: $$(grep -c 'function' src/async_result.ads)$(NC)"; \
		echo "$(YELLOW)📊 Generic packages: $$(grep -c 'package.*is' src/async_result.ads)$(NC)"; \
	fi
...
docs-check:
	...
	@documented_items=$$(grep -c "^[[:space:]]*--[[:space:]]*@" src/async_result.ads 2>/dev/null || echo "0"); \
	public_items=$$(grep -c "^[[:space:]]*(function|procedure|type)" src/async_result.ads 2>/dev/null | grep -v "private" || echo "0"); \
